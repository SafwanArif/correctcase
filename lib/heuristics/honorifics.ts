...
const honorifics = new Set([
    'president', 'vice president', 'doctor', 'dr', 'mr', 'mrs', 'miss', 'ms', 'mx',
    'professor', 'prof', 'aunt', 'uncle', 'auntie', 'sister', 'father', 'mother', 'brother',
    'pope', 'rabbi', 'imam', 'bishop', 'cardinal',
    'senator', 'governor', 'representative', 'congressman', 'congresswoman', 'mp',
    'chancellor', 'premier', 'prime minister', 'minister', 'ambassador',
    'king', 'queen', 'prince', 'princess', 'duke', 'duchess', 'emperor', 'empress',
    'general', 'captain', 'major', 'lieutenant', 'colonel', 'sergeant', 'detective', 'inspector', 'constable'
]);

const riskyHonorifics = new Set(['will', 'mark', 'bill', 'major']);
const determiners = new Set(['the', 'a', 'an', 'my', 'our', 'your', 'his', 'her', 'their', 'this', 'that', 'these', 'those']);
...
const commonVerbs = new Set(['is', 'are', 'was', 'were', 'has', 'have', 'had', 'said', 'asked', 'went', 'did', 'do', 'does', 'can', 'could', 'should', 'would', 'will', 'may', 'might', 'must']);
    ...
    if (!precededByDeterminer && !riskyHonorifics.has(lowerKey)) {
        ...
        if (commonVerbs.has(nextKey)) {
            ...
            if (!US_MINOR_WORDS.has(nextKey) && nextKey !== 'of') {
                ...
                const capNext = nextP.word.charAt(0).toUpperCase() + nextP.word.slice(1).toLowerCase();
