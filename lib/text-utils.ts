...
import {
    GLOBAL_COMPOUND_WORDS,
    SENTENCE_CASE_EXCEPTIONS_MAP,
    US_MINOR_WORDS,
    PHRASAL_VERB_ROOTS,
            ...
            .map((word, index) => {
                ...
                if (index > 0) {
                    ...
                    if (stem.endsWith("ing")) {
                        ...
                        else if (PHRASAL_VERB_ROOTS.has(rawStem.slice(0, -1)))
                            stem = rawStem.slice(0, -1);
                    }
...
export function smartUnhyphenate(text: string): string {
    ...
    while (i < parts.length) {
        ...
        if (i < parts.length - 3) {
            ...
            if (GLOBAL_COMPOUND_WORDS.has(candidate)) {
